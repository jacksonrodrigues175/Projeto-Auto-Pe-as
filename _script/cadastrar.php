...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES['imagem']) && is_uploaded_file($_FILES['imagem']['tmp_name'])) {
    ...
    // Validate user input
    if (empty($nomePeca) || empty($fornecedor) || empty($valorCompra) || empty($valorVenda) || empty($quantidade) || empty($dataCadastro)) {
    ...
    $stmt->bind_param("ssddsss", $nomePeca, $fornecedor, $valorCompra, $valorVenda, $quantidade, $dataCadastro, $target_file);
    $stmt->execute();
...
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" class="m-5" enctype="multipart/form-data">
    <div class="form-group">
        <label for="nomePeca">Nome da Peça:</label>
        <input type="text" id="nomePeca" name="nomePeca" class="form-control">
    </div>
    <div class="form-group">
