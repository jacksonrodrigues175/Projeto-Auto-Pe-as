...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate user input
    if (empty($nomePeca) || empty($fornecedor) || empty($valorCompra) || empty($valorVenda) || empty($dataCadastro) || empty($imagem)) {
    ...
    $sql = "INSERT INTO Pecas (nomePeca, fornecedor, valorCompra, valorVenda, dataCadastro, imagem) VALUES (?,?,?,?,?,?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssddss", $nomePeca, $fornecedor, $valorCompra, $valorVenda, $dataCadastro, $target_file);
    $stmt->execute();
        ...
        <div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 justify-content-center">
            <div class="col mb-5">
                <div class="card h-100">
                    ...
                    $stmt = $conn->prepare("SELECT id, nomePeca, valorVenda, imagem FROM Pecas");
                    $stmt->execute();
